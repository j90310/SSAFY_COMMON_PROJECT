...
class BuyDetailGuest extends StatefulWidget {
  final Party party;
  final writer;
  const BuyDetailGuest({Key? key, required this.party, required this.writer})
      : super(key: key);
...
class _BuyDetailGuestState extends State<BuyDetailGuest> {
  ...
  Future insertReport(Report report) async {
    print(
        'report: ${report.crimeName}, ${report.reportContent}, ${report.reportedUserSeq}, ${report.reportingUserSeq}, ${report.reportSeq}');
  ...
  void loadSetState(partyProvider, partySeq) async {
    ...
    widget.party.partyMemberNumCurrent =
        partyProvider.currentParty.partyMemberNumCurrent;
  ...
  Widget build(BuildContext context) {
    ...
    void _createChatRoom() async {
      String? chatName = 'H' +
          widget.party.userResVO.userKakaoLoginId.toString() +
          "G" +
          kakaoUserProvider.userResVO!.userKakaoLoginId.toString() +
          "P" +
          widget.party.partySeq.toString();

      await _database.collection('chats').doc(chatName).set({
        'created_at': Timestamp.now(),
        'guestNickname':
            kakaoUserProvider.user?.kakaoAccount?.profile?.nickname,
                ...
                child: Container(
                  ...
                  child: LikeButton(
                    ...
                    likeBuilder: (bool isLiked) {
                      return Icon(
                        bookmarkList.contains(widget.party.partySeq)
                            ? Icons.favorite
                    ...
                    isLiked: bookmarkList.contains(widget.party.partySeq)
                        ? true
                        : false,
                          ...
                          child: SizedBox.expand(
                            child: OutlinedButton(
                              ...
                              style: OutlinedButton.styleFrom(
                                ...
                                side:
                                    BorderSide(width: 5.0, color: Colors.amber),
                              ),
                              child: Text(
                                '대화 걸기',
                                style: TextStyle(
